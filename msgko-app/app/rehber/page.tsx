      ...
      <main style={{ minHeight: '100vh', background: 'var(--void)' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', maxWidth: '1320px', margin: '0 auto', padding: '2rem clamp(1.25rem,4vw,2.5rem) 3rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <div style={{ width: 20, height: 1, background: '#C8102E' }} />
              <span style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '0.32em', textTransform: 'uppercase', color: '#E8192F', opacity: 0.85 }}>
                Rehber Merkezi
              </span>
            </div>
          ...
          {/* Alt kırmızı çizgi */}
          <div style={{ height: 1, background: 'linear-gradient(90deg, #C8102E, rgba(255,77,28,0.5), transparent)' }} />
        </div>
        ...
        <section style={{ maxWidth: '1320px', margin: '0 auto', padding: '3rem clamp(1.25rem,4vw,2.5rem) 5rem' }}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {KO_CLASSES.map(cls => {
              const acc = CLASS_ACCENT[cls.slug] ?? { border: 'rgba(200,16,46,0.2)', bg: 'rgba(200,16,46,0.05)', label: '' }
              return (
                <Link
                  key={cls.slug}
